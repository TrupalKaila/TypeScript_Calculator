const angleModes: string[] = ["DEG", "RAD", "GRAD"];
let currentAngleIndex: number = 0;
...
function toggleSecondInverse(): void {
...
function updateTrigButtons() {
    ...
    buttonname.forEach(({ id, base }) => {
        ...
        el.innerHTML = label;
        (el as HTMLButtonElement).onclick = () => calculateTrig(funcName);
    });
}

function calculateTrig(funcName: string): void {
